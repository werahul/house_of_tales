...
const faqs: FAQItem[] = [
  ...
  },
  {
    ...
    answer: "We capture a mix of candid andcontemporary photos, focusing on real, heartfelt moments shared by the couple,families, and guests. You’ll receive all your edited photos, a 3–5 minute cinematictrailer, reels (on request), and full film of your wedding and other celebrations likeHaldi, Sangeet, etc.",
  },
  {
...
const FAQSection: React.FC = () => {
  ...
  return (
    ...
    >
      <h2 className="text-[47px] mirage text-[#34291E] text-center mb-8">FAQS</h2>
      <div className="max-w-3xl mx-auto space-y-4">
        {faqs.map((faq, index) => (
          <motion.div
            key={index}
            layout
            className={`border rounded-md px-6 py-4 cursor-pointer bg-white ${activeIndex === index ? "border-[#d4cdbf]" : "border-transparent"
              }`}
            onClick={() => toggleIndex(index)}
          >
            <div className="flex items-center justify-between">
              <h3
                className={`text-[21px] mirage antialiased ${activeIndex === index
